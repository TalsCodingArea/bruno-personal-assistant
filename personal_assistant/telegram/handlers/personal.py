...
async def handle_personal_assistant_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        intent = await classify_intent(runtime.llm, user_text)
    except Exception:
        intent = "general"
    if intent == "finance":
        status_msg = await message.reply_text("📈 Evaluating your finances...")
        stop_typing = asyncio.Event()
        typing_task = asyncio.create_task(_keep_typing(context.bot, message.chat_id, stop_typing))
        try:
            result = await runtime.run_financial_advisor(chat_id, user_text)
            response = result.get("output", "")
        except Exception as exc:
            logger.exception("Financial advisor graph error for chat %s", chat_id)
            await message.reply_text("Something went wrong with the financial advisor. Please try again.")
            await safe_log(context, f"[financial_advisor:error] {exc}")
            return
        finally:
            stop_typing.set()
            await typing_task
